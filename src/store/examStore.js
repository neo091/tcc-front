...
export const useExamStore = create(persist((set, get) => ({
    ...
    goNextTest: () => {
        const { currentQuest, questions } = get()
        const nextTest = currentQuest + 1

        if (nextTest < questions.length) {
            set({ currentQuest: nextTest })
        }
    ...
    goPrevTest: () => {
        ...
        const prevTest = currentQuest - 1

        if (prevTest >= 0) {
            set({ currentQuest: prevTest })
        }
    },
    startToNext: (time) => {
    ...
    reset: () => {
        set({ questions: [], currentQuest: 0, replied: 0 })
    },
